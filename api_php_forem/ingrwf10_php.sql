...
--
-- Hôte : 127.0.0.1
-- Généré le : dim. 10 juil. 2022 à 17:22
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `marcel`  AS SELECT `personnes`.`id_personnes` AS `id_personnes`, `personnes`.`nom` AS `nom`, `personnes`.`prenom` AS `prenom` FROM `personnes` WHERE `personnes`.`prenom` like '%marcel%''%marcel%'  ;
